...
> [!IMPORTANT] 
> We created a special branch for the SOSP24 artifact evaluation process. Reviewers are invited clone this repository, switch to the `sosp24-ae` branch, and follow the guidelines in [sosp24_ae_instructions.md](https://github.com/bastoica/wasabi/blob/sosp24-ae/sosp24_ae_instructions.md) for a step-by-step walkthrough over our benchmarks. While we sync the `master` branch reguraly, the most up-to-date instructions relevant to the AE committee are published on the `sosp24-ae` branch first.
