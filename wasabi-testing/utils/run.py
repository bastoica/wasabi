...
def run_fault_injection(root_dir, application_name):
  ...
  cmd = ["python3", " run_benchmark.py", "--benchmark", application_name]
...
def save_output_to_file(root_dir, phase, output):
  ...
  with open(filename, "w") as f:
    f.write(output)
  print(f"[WASABI-HELPER]: [INFO]: Output saved to {filename}")
...
def main():
  ...
  root_dir = os.getenv("WASABI_ROOT_DIR")
  if not root_dir:
    print("[WASABI-HELPER]: [ERROR]: The WASABI_ROOT_DIR environment variable is not set.")
    sys.exit(1)

  if benchmarks == "all-maven":
    benchmark_list = ["hadoop", "hbase", "hive", "cassandra"]
  else:
