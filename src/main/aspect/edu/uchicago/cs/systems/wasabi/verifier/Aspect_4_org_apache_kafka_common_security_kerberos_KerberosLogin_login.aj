...
public aspect Aspect_4_org_apache_kafka_common_security_kerberos_KerberosLogin_login {
    ...
    pointcut requestMethodOnly():
        (execution(* org.apache.kafka.common.security.kerberos.KerberosLogin.reLogin(..)));

    pointcut requestMethod():
        (cflow(execution(* org.apache.kafka.common.security.kerberos.KerberosLogin.login(..))) && execution(* org.apache.kafka.common.security.kerberos.KerberosLogin.reLogin(..)));
    ...
    after() throws javax.security.auth.login.LoginException : requestMethodOnly() {
        ...
        if (requestAttempts <= NUM_FAILURES_TO_INJECT) {
            ...
            throw new javax.security.auth.login.LoginException("wasabi exception from " + thisJoinPoint);
        } else {
