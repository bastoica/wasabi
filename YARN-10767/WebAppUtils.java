...
public class WebAppUtils {
  ...
  public static <T, R> R execOnActiveRM(Configuration conf,
    ...
    LOG.info("[wasabi] WebAppUtils 105");
    try {
      LOG.info("[wasabi] WebAppUtils 107");
      return func.apply(rm1Address, arg);
    } catch (Exception e) {
      LOG.info("[wasabi] WebAppUtils 110");
      if (HAUtil.isHAEnabled(conf)) {
        LOG.info("[wasabi] WebAppUtils 112");
        int rms = HAUtil.getRMHAIds(conf).size();
        for (int i=1; i<rms; i++) {
          try {
            LOG.info("[wasabi] WebAppUtils 116");
            rm1Address = getRMWebAppURLWithScheme(conf, i);
            return func.apply(rm1Address, arg);
          } catch (Exception e1) {
            // ignore and try next one when RM is down
            LOG.info("[wasabi] WebAppUtils 121");
            e = e1;
          }
        }
      }
